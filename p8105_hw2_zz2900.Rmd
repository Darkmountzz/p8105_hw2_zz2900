...
For the first data set, Mr. Trash Wheel contains ``r nrow(Trash_data)`` observations with ``r ncol(Trash_data)`` variables which are respectively `Trash_data`(``r names(Trash_data)``)

For the second data set, Professor Trash Wheel contains ``r nrow(Professor_data)`` observations with ``r ncol(Professor_data)`` variables which are respectively `Professor_data`(``r names(Professor_data)``)

For the merge data set, it contains ``r nrow(merge_data)`` observations with ``r ncol(merge_data)`` variables which are respectively `merge_data`(``r names(merge_data)``)
